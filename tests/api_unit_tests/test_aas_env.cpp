...
TEST_F(TestAasEnv, TestPut)
{
	...
	// Put a single AAS
	auto put_aas = envApi.putAssetAdministrationShell(aas_1);
	...
	auto no_put_aas = envApi.putAssetAdministrationShell(aas_1);
	ASSERT_EQ(envApi.getAssetAdministrationShells().size(), 1);
	ASSERT_EQ(no_put_aas, nullptr);

	// Put a second AAS
	auto put_aas_2 = envApi.putAssetAdministrationShell(aas_2);
